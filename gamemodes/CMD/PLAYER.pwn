...
CMD:sid(playerid, params[])
{
	 if(!pData[playerid][pTogName])
    {
        ...
        foreach(new pid : Player)
        {
            ...
            playerID[playerid] = CreateDynamic3DTextLabel(nametag, 0xFFFFFFFF, 0.0, 0.0, 0.1, NT_DISTANCE, .attachedplayer = pid, .testlos = 1);
            pData[playerid][pTogName] = 1;
        }
    ...
    else
    {
        ...
        if(IsValidDynamic3DTextLabel(playerID[playerid]))
          DestroyDynamic3DTextLabel(playerID[playerid]);
    }
    return 1;
}
...
{
    ShowPlayerDialog(playerid, DIALOG_HBEMODE, DIALOG_STYLE_LIST, "Asialane - HBE Mode", ""LG_E"Simple\n"LG_E"Modern", "Pilih", "Tutup");
    return 1;
}
...
stock Altruqtd(playerid, string[], time)//Time in Sec.
{
	...
	PlayerTextDrawSetString(playerid, Alttdruq[playerid][9], string);
...
//nasi kucing
CMD:mamnasikucing(playerid, params[])
{
    if(pData[playerid][pNasikucing] < 1) return ErrorMsg(playerid,"Anda tidak memiliki nasi kucing.");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pNasikucing]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Nasi Kucing", "Removed_1x", 2768, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Makan Roti..",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Makan Nasi Kucing..", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
	    SetPlayerAttachedObject(playerid, 3, 2768, 6, 0.000000, -0.025000, 0.066000, 0.000000, 0.000000, 0.000000, 1.000000, 1.000000, 1.000000);
		pData[playerid][pHunger] += 10;
	    return 1;
    }
}
CMD:mamayamgeprek(playerid, params[])
{
    if(pData[playerid][pAyamgeprek] < 1) return ErrorMsg(playerid,"Anda tidak memiliki Ayam Geprek.");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pAyamgeprek]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Ayam Geprek", "Removed_1x", 2768, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Makan Roti..",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Makan Ayam Geprek.", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
	    SetPlayerAttachedObject(playerid, 3, 2768, 6, 0.000000, -0.025000, 0.066000, 0.000000, 0.000000, 0.000000, 1.000000, 1.000000, 1.000000);
...
CMD:mamkue(playerid, params[])
{
    if(pData[playerid][pKueaufa] < 1) return ErrorMsg(playerid,"Anda tidak memiliki kue.");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pKueaufa]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Kue", "Removed_1x", 19525, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Makan Kue...",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Makan Ayam Geprek.", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
	    SetPlayerAttachedObject(playerid, 3, 19525, 6, 0.000000, -0.025000, 0.066000, 0.000000, 0.000000, 0.000000, 1.000000, 1.000000, 1.000000);
		pData[playerid][pHunger] += 30;
	    return 1;
    }
}
CMD:numkopi(playerid, params[])
{
    if(pData[playerid][pKopiruq] < 1) return ErrorMsg(playerid,"Anda tidak memiliki Kopi");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pKopiruq]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Kopi", "Removed_1x", 19835, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Minum Kopi..",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Minum Kopi...", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
...
CMD:numesteh(playerid, params[])
{
    if(pData[playerid][pEsteh] < 1) return ErrorMsg(playerid,"Anda tidak memiliki Esteh");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pEsteh]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Esteh", "Removed_1x", 2647, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Makan Roti..",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Minum Esteh...", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
	    SetPlayerAttachedObject(playerid, 3, 2647, 6, 0.000000, -0.025000, 0.066000, 0.000000, 0.000000, 0.000000, 1.000000, 1.000000, 1.000000);
		pData[playerid][pEnergy] += 17;
	    return 1;
    }
}
CMD:numesjeruk(playerid, params[])
{
    if(pData[playerid][pEsjeruk] < 1) return ErrorMsg(playerid,"Anda tidak memiliki EsJeruk");
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Tunggu Sebentar");
	if(pData[playerid][sampahsaya] == 10) return ErrorMsg(playerid, "Buang sampah anda terlebih dahulu");
    {
	    pData[playerid][pEsjeruk]--;
	    pData[playerid][sampahsaya]++;
	    Inventory_Update(playerid);
        Inventory_Close(playerid);
        ShowItemBox(playerid, "Esjeruk", "Removed_1x", 19563, 2);
		ShowItemBox(playerid, "Sampah", "Received_1x", 1265, 2);
	    SetPlayerChatBubble(playerid,"> Makan Roti..",COLOR_PURPLE,30.0,10000);
	    ShowProgressbar(playerid, "Minum Esjeruk...", 1);
		ApplyAnimation(playerid, "FOOD", "EAT_Burger", 4.0, 0, 0, 0, 0, 0,1);
	    SetPlayerAttachedObject(playerid, 3, 19563, 6, 0.000000, -0.025000, 0.066000, 0.000000, 0.000000, 0.000000, 1.000000, 1.000000, 1.000000);
		pData[playerid][pEnergy] += 19;
		RemovePlayerAttachedObject(playerid, 9);
	    return 1;
    }
}
...
CMD:i(playerid, params[])
{
	...
	SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "ACTION: %s membuka inventory", ReturnName(playerid));
	return 1;
}
CMD:h(playerid, params[])
{
	...
	SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "ACTION: %s membuka hp", ReturnName(playerid));
	return 1;
}
	...
	foreach(new ii : Player)
	{
		if(pData[ii][pPhone] == ph)
		{
			...
			if(pData[ii][pCall] == INVALID_PLAYER_ID)
			{
				...
				for(new i = 0; i < 62; i++)
		        {
			        TextDrawHideForPlayer(ii, PhoneHome[i]);
		        }
		        TextDrawHideForPlayer(ii, CLOSE);
		        TextDrawHideForPlayer(ii, JAM);
				...
				PlayerTextDrawShow(ii, BerderingtelHP[ii]);
...
CMD:giveidcard(playerid, params[])
{
	...
	PlayerTextDrawSetString(playerid, ktpaufa[playerid][10], strings);
	// Set birtdate
	format(strings, sizeof(strings), "%s", pData[playerid][pAge]);
	PlayerTextDrawSetString(playerid, ktpaufa[playerid][14], strings);
	// Set Job
	format(strings, sizeof(strings), "%s", fac);
	PlayerTextDrawSetString(playerid, ktpaufa[playerid][16], strings);
	// Set Expired IDCARD
	format(strings, sizeof(strings), "%s", ReturnTimelapse(gettime(), pData[playerid][pIDCardTime]));
	PlayerTextDrawSetString(playerid, ktpaufa[playerid][18], strings);
	// // Set Skin Player
	...
	{
		PlayerTextDrawShow(playerid, ktpaufa[playerid][txd]);
...
CMD:help(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_HELP, DIALOG_STYLE_TABLIST_HEADERS, "Asialane - Bantuan", info, "Pilih", "Tutup");
	return 1;
}
...
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Website: "YELLOW_E"https://discord.gg/Asialanerp\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"GREEN_E"Terima kasih telah bergabung dengan kami! Copyright © 2022 | Asialane Roleplay.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Asialane:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
CMD:vip(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Asia Lane "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
// CMD:gantinama(playerid, params[]) {
//     if(IsAtEvent[playerid] == 1)
//         return ErrorMsg(playerid, "Anda sedang mengikuti event & tidak bisa melakukan ini");

//     // new Dstring[512];
//     // // Gunakan satu pemanggilan format() dan tambahkan setiap opsi pembelian ke string Dstring
//     // format(Dstring, sizeof(Dstring), "Asialane TOP-UP\tPrice\n");
//     // format(Dstring, sizeof(Dstring), "%sGanti Nama\t800 Coin\n", Dstring);
...
//     ShowPlayerDialog(playerid, DIALOG_GOLDSHOP, DIALOG_STYLE_TABLIST_HEADERS, "Ganti Nama | Asialane", Dstring, "Ganti", "Cancel");
//     return 1;
// }


CMD:pku(playerid, params[])
{
...
CMD:death(playerid, params[])
{
    ...
    if((gettime()-GetPVarInt(playerid, "GiveUptime")) < 600)
        return ErrorMsg(playerid, "Kamu harus menunggu 10 menit untuk bisa /death");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1160.23, -1299.97, 13.61)) return ErrorMsg(playerid, "Anda harus berada di rumah sakit!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1571.9326,-1691.6022,16.2153)) return ErrorMsg(playerid, "Anda harus berada di Kantor kepolisian!");
	if(pData[playerid][pDriveLic] != 0) return ErrorMsg(playerid, "Anda sudah memiliki sim!");
	if(GetPlayerMoney(playerid) < 700) return ErrorMsg(playerid, "Anda butuh $1200 untuk membuat sim.");
	pData[playerid][pDriveLic] = 1;
	pData[playerid][pDriveLicTime] = gettime() + (30 * 86400);
	GivePlayerMoneyEx(playerid, -1200);
	ShowItemBox(playerid, "Uang", "REMOVED_$1200", 1212, 4);
	Server_AddMoney(1200);
	SuccesMsg(playerid, "Selamat Anda telah berhasil membuat SIM");
	return 1;
}
CMD:payticket(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1571.9326,-1691.6022,16.2153) && !IsPlayerInRangeOfPoint(playerid, 3.0, 1571.9326,-1691.6022,16.2153)) return ErrorMsg(playerid, "Anda harus berada di kantor SAPD!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1571.9326,-1691.6022,16.2153)) return ErrorMsg(playerid, "Anda harus berada di Kantor Polisi!");
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				format(pvData[i][cPlate], 32, "ALRP-%d", rand);
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1530.5714,-2159.6865,13.6593)) return ErrorMsg(playerid, "Kamu harus berada di Kantor Insurance!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 935.6656,-1670.8666,14.0791)) return ErrorMsg(playerid, "Anda harus berada di Bank!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 942.2501,-1667.1718,14.0791)) return ErrorMsg(playerid, "Anda harus berada di bank point!");
	new tstr[128];
...
CMD:workd(playerid, params[]) {
	...
	if(!IsPlayerInRangeOfPoint(playerid, 1.0, 2351.7666, -658.1649, 128.0146)) {
		...
		if(inJOB[playerid] == 1)
			return SendClientMessage(playerid, ATENTIE, "Error: Terjadi kesalahan!");

		inJOB[playerid] = 1;
		new rand = random(3);

		switch(rand) {
			case 1: Car_Job[playerid] = CreateVehicle(505, 2406.9773, -403.4681, 72.4926, -180.5998, 225, 142, 100);
			case 2: Car_Job[playerid] = CreateVehicle(505, 2406.9773, -403.4681, 72.4926, 1.5600, 120, 131, 100);
...
//     if(sscanf(params, "us[24]d", otherid))
//     {
//         SyntaxMsg(playerid, "/givektp [playerid]");
//         return 1;
//     }

//     if(strcmp(name,"KTP",true) == 0) 
//     {
...
//         pData[otherid][pIDCard] += ammount;
//         pData[otherid][pIDCardTime] = gettime() + (30 * 86400);
//         Info(playerid, "Anda telah berhasil memberikan KTP kepada %s", ReturnName(otherid), ammount);
//         Info(otherid, "%s telah berhasil memberikan KTP kepada anda.", ReturnName(playerid), ammount);
//         ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
//         ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);  
//         return 1;
//     }

//     // Tambahkan pernyataan "return" dengan nilai yang sesuai di akhir fungsi
//     return 0;
// }
...
CMD:give(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]d", otherid, name, ammount))
		{
			...
			Info(playerid, "Names: bandage, ayamfillet, cigarette, redmoney, nasikucing, ayamgeprek, kue");
		...
		else if(strcmp(name,"nasikucing",true) == 0)
		{
			if(pData[playerid][pNasikucing] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");

			pData[playerid][pNasikucing] -= ammount;
			pData[otherid][pNasikucing] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Nasi Kucing kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Nasi Kucing kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"ayamgeprek",true) == 0)
		{
			if(pData[playerid][pAyamgeprek] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");

			pData[playerid][pAyamgeprek] -= ammount;
			pData[otherid][pAyamgeprek] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Ayam geprek kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Ayam geprek kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"kue",true) == 0)
		{
			if(pData[playerid][pKueaufa] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");

			pData[playerid][pKueaufa] -= ammount;
			pData[otherid][pKueaufa] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Kue kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Kue kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
...
//give ke pedagang
CMD:givehasiltani(playerid, params[])
{
	if(IsAtEvent[playerid] == 1)
		return Error(playerid, "Anda sedang mengikuti event & tidak bisa melakukan ini");
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		{
			SyntaxMsg(playerid, "/give [playerid] [name] [ammount]");
			Info(playerid, "Names: beras, sambal, tepung, gula, bijikopi, sariteh, sarijeruk");
			return 1;
		}
		if(otherid == INVALID_PLAYER_ID || otherid == playerid || !NearPlayer(playerid, otherid, 3.0))
			return Error(playerid, "Invalid playerid!");
			
		if(strcmp(name,"beras",true) == 0) 
		{
			if(pData[playerid][pBeras] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pBeras] -= ammount;
			pData[otherid][pBeras] += ammount;
			Info(playerid, "Anda telah berhasil memberikan beras kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan beras kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"sambal",true) == 0) 
		{
			if(pData[playerid][pSambal] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pSambal] -= ammount;
			pData[otherid][pSambal] += ammount;
			Info(playerid, "Anda telah berhasil memberikan sambal kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan sambal kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"tepung",true) == 0) 
		{
			if(pData[playerid][pTepung] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pTepung] -= ammount;
			pData[otherid][pTepung] += ammount;
			Info(playerid, "Anda telah berhasil memberikan tepung kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan tepung kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"gula",true) == 0) 
		{
			if(pData[playerid][pGula] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pGula] -= ammount;
			pData[otherid][pGula] += ammount;
			Info(playerid, "Anda telah berhasil memberikan gula kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan gula kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"bijikopi",true) == 0) 
		{
			if(pData[playerid][pBijikopi] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pBijikopi] -= ammount;
			pData[otherid][pBijikopi] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Biji kopi kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Biji Kopi kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"sariteh",true) == 0) 
		{
			if(pData[playerid][pSariteh] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pSariteh] -= ammount;
			pData[otherid][pSariteh] += ammount;
			Info(playerid, "Anda telah berhasil memberikan sari teh kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan sari teh kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"sarijeruk",true) == 0) 
		{
			if(pData[playerid][pSarijeruk] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pSarijeruk] -= ammount;
			pData[otherid][pSarijeruk] += ammount;
			Info(playerid, "Anda telah berhasil memberikan sari jeruk kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan sari jeruk kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
	}
...
CMD:pgive(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		{
			SyntaxMsg(playerid, "/give [playerid] [name] [ammount]");
			Info(playerid, "Names: nasikucing, ayamgeprek, kue, kopi, esteh, esjeruk");
			return 1;
		}
		if(otherid == INVALID_PLAYER_ID || otherid == playerid || !NearPlayer(playerid, otherid, 3.0))
			return Error(playerid, "Invalid playerid!");
			
		if(strcmp(name,"nasikucing",true) == 0) 
		{
			if(pData[playerid][pNasikucing] < ammount)
				return Error(playerid, "Item anda tidak cukup.");
		...
		else if(strcmp(name,"kopi",true) == 0) 
		{
			if(pData[playerid][pKopiruq] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pKopiruq] -= ammount;
			pData[otherid][pKopiruq] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Kopi kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Kopi kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"esteh",true) == 0) 
		{
			if(pData[playerid][pEsteh] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pEsteh] -= ammount;
			pData[otherid][pEsteh] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Es Teh kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan Es Teh kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"esjeruk",true) == 0) 
		{
			if(pData[playerid][pEsjeruk] < ammount)
				return Error(playerid, "Item anda tidak cukup.");

			if(ammount < 1) return Error(playerid, "Can't Give below 1");
			
			pData[playerid][pEsjeruk] -= ammount;
			pData[otherid][pEsjeruk] += ammount;
			Info(playerid, "Anda telah berhasil memberikan Es jeruk kepada %s sejumlah %d.", ReturnName(otherid), ammount);
			Info(otherid, "%s telah berhasil memberikan ES jeruk kepada anda sejumlah %d.", ReturnName(playerid), ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
	}
...
CMD:b(playerid, params[])
{
	...
	else
	{
        ...
        {
            SendNearbyMessage(playerid, 20.0, COLOR_WHITE, "[L] %s {ffffff}%s [%d]: (( %s ))", tierName, pData[playerid][pUCP], playerid, params);
            return 1;
        }
	}
...
CMD:iklan(playerid, params[])
{
	...
	// if(IsPlayerInRangeOfPoint(playerid, 50, 2060.9265,1338.6526,1003.5000))
	// 	return Error(playerid, "Anda tidak dapat melakukan ini jika sedang berada di OOC Zone");
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 2060.9265,1338.6526,1003.5000)) return Error(playerid, "Kamu harus di kantor berita");
	if(pData[playerid][pDelayIklan] > 0) return Error(playerid, "Kamu masih cooldown %d detik", pData[playerid][pDelayIklan]);
	if(pData[playerid][pPhone] == 0) return Error(playerid, "Anda tidak memiliki Ponsel!");
	
	if(isnull(params))
	{
		SyntaxMsg(playerid, "/iklan [Pesan] | 1 huruf bayar $2");
		return 1;
	}
	if(strlen(params) >= 100 ) return Error(playerid, "maximal pesan 100 huruf." );
	new payout = strlen(params) * 2;
	if(GetPlayerMoney(playerid) < payout) return Error(playerid, "kamu tidak memiliki uang");
...
{	
	// if(pData[playerid][pGPS] < 1) return Error(playerid, "Ketik /fixgps Untuk Menyalakan GPS");
...
CMD:fixgps(playerid, params[])
{
	if(pData[playerid][pGPS] != 0)
		return ErrorMsg(playerid, "Kamu sudah mengambil Sebelumnya!");

	pData[playerid][pGPS] = 1;
	SuccesMsg(playerid, "Selamat GPS Anda Telah Berfungsi Kembali");
	ShowItemBox(playerid, "GPS", "Memperbaiki GPS", 1581, 4);
    return 1;
}
...
DelaysPlayer(playerid, p2)
{
    ...
    {
        format(str, sizeof(str), "%s\nBus (Sidejob)\t%i Second\n", str, pData[p2][pBusTime]);
	}
	if(pData[p2][pSparepartTime] > 0)
    {
        format(str, sizeof(str), "%s\nJob Sparepart (Jobs)\t%i Second\n", str, pData[p2][pSparepartTime]);
	}
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "Delays", str, "Okay", "");
...
// {
// 	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1382.1221,1531.8909,1510.9011)) return Error(playerid, "Anda harus berada di Kantor SAPD!");
// 	if(pData[playerid][pDriveLic] != 0) return Error(playerid, "Anda sudah memiliki Weapon License!");
// 	if(GetPlayerMoney(playerid) < 500000) return Error(playerid, "Anda butuh $500,000 untuk membuat Weapon License.");
...
// CMD:tutupmata(playerid,params[])
// {
//     new targetid;
// 	if(sscanf(params, "u", targetid))
// 	{
// 	    return SyntaxMsg(playerid, "/tutupmata [playerid]");
// 	}
// 	if(pData[playerid][pBandana] <= 0)
// 	{
...
// 	{
// 	    return Error(playerid, "Orang itu sudah ditutup matanya. '/bukatutupmata' untuk melepas.");
// 	}
...
// CMD:bukatutupmata(playerid, params[])
// {
// 	new targetid;

// 	if(sscanf(params, "u", targetid))
// 	{
// 	    return SCM(playerid, COLOR_SYNTAX, "Gunakan: /bukatutupmata [playerid]");
// 	}
...
{
	new lstr[1024];
	format(lstr, sizeof(lstr), "Nama Organisasi\tJumlah\nTenaga Medis Asialane\t%d\nKepolisian Asialane\t%d\nBengkel Asialane\t%d\nReporter Asialane\t%d\nTaxi Asialane\t%d\nPedagang Asialane\t%d", emsdikota, polisidikota, bengkeldikota, reporterdikota, gojekdikota, pedagangdikota);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "Asialane - Organisasi Online", lstr, "Tutup", "");
	return 1;
}
CMD:makesks(playerid, params[])
{
	...
	new string[1000];
	format(string, sizeof(string), ""YELLOW_E"======SURAT KETERANGAN SEHAT=====\nNama : %s\nTanggal lahir : %s\nBerat Badan : %s\nTinggi Badan : %s\nMenyatakan bahwa:\nSurat ini sah hingga masa berlaku yang telah di tentukan", pData[playerid][pName], pData[playerid][pAge], pData[playerid][pBerat], pData[playerid][pTinggi]);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Memberi Surat Keterangan sehat", string, "Close","");
    return 1;
}
...
CMD:givesks(playerid, params[])
{
    ...
    new str[500];
    format(str, sizeof(str), "samd %s telah memberikan anda surat .", ReturnName(playerid));
    InfoMsg(otherid, str);
    return 1;
}
...
// {
//     if(pData[playerid][pKompensasi1] != 0) return ErrorMsg(playerid, "Kamu sudah mengambil kompensasi, jangan ngambil lagi !!!!!!");
// 	{
// 		pData[playerid][pKompensasi1] = 1;
// 		pData[playerid][pSnack] += 10;
// 		pData[playerid][pSprunk] += 10;
// 		GivePlayerMoneyEx(playerid, 4000);
// 		SuccesMsg(playerid, "Anda Diberi Kompensasi 20 Snack, 20 Water Dan $4.000");
// 		SendNearbyMessage(playerid, 20.0, COLOR_PURPLE, "* %s telah mengambil kompensasi", ReturnName(playerid));
// 		SendClientMessage(playerid, -1, "Selamat Anda berhasil mendapatkan kompensasi maintenace dari Gyu");
// 		ShowItemBox(playerid, "Snack", "Received_20x", 2821, 4);
// 		ShowItemBox(playerid, "Water", "Received_20x", 2958, 4);
// 		ShowItemBox(playerid, "Uang", "Received_$4.000", 1212, 4);
// 	}
// 	return 1;
// }
