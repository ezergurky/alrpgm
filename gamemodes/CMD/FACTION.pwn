...
CMD:factionhelp(playerid)
{
	if(pData[playerid][pFaction] == 1)
	{
		...
		strcat(str, ""BLUE_E"SAPD: /takemarijuana /spike /destroyspike /destroyallspike /getloc //roadblockn");
	...
	else if(pData[playerid][pFaction] == 2)
	{
		...
		strcat(str, ""WHITE_E"SAGS: /sagsonline /(un)cuff /checkcitymoney /roadblock /createhouse\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5, 1090.9022,1318.3073,10.9625)) return ErrorMsg(playerid, "Kamu harus di dapur!");
	...
	new Dstring[512];
	format(Dstring, sizeof(Dstring), "Jenis\tBahan\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sNasi Kucing\tBeras(5) + Sambal(2)\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sAyam Geprek\tAyam(5) + Sambal(2)\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sCake\tTepung(5) + Gula(5)\n", Dstring);
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 2.5, 1087.5863,1318.2523,10.9623)) return ErrorMsg(playerid, "Kamu harus di kulkas!");
	if(pData[playerid][pLevel] < 2) return ErrorMsg(playerid, "level kamu harus 2!.");
	if(pData[playerid][pFaction] != 5)
        return ErrorMsg(playerid, "Anda harus menjadi seorang pedagang");

	new Dstring[512];
	format(Dstring, sizeof(Dstring), "Minuman\tBahan\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sKopi\tMineral(1) + Gula(2) + Biji Kopi(2)\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sEs Teh\tMineral(1) + Gula(2) + sari teh(2)\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sEs Jeruk\tMineral(1) + Gula(2) + sari jeruk(2)\n", Dstring);
...
ShowPedagangMenu(playerid, pid)
{
    ...
    new str[256], vstr[64];
    format(vstr, sizeof vstr,"{BABABA}Asialane - {FFFFFF}Gudang Pedagang");
    format(str, sizeof str,"Item\tStok\nAyam\t(%d)\n{BABABA}Beras\t{BABABA}(%d)\n{FFFFFF}Sambal\t(%d)\n{BABABA}Tepung\t{BABABA}(%d)\n{FFFFFF}Gula\t(%d)\n{BABABA}Biji Kopi\t(%d)\n{FFFFFF}Sari Teh\t(%d)\n{BABABA}Sari Jeruk\t(%d)\n{FFFFFF}Mineral\t(%d)",
        pdgDATA[pid][pdgAyamfilet],
		pdgDATA[pid][pdgBeras],
		pdgDATA[pid][pdgSambal],
		pdgDATA[pid][pdgTepung],
		pdgDATA[pid][pdgGula],
		pdgDATA[pid][pdgBijikopi],
		pdgDATA[pid][pdgSariteh],
		pdgDATA[pid][pdgSarijeruk],
...
{
		TextDrawShowForPlayer(playerid, CLOSEPMENU);
	
		for(new i = 0; i < 40; i++)
		{
			TextDrawShowForPlayer(playerid, PMENURUQ[i]);
...
CMD:release(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1364.0143,1577.2247,1468.7877))
	...
	SetPlayerPositionEx(otherid, 1508.1083,-1671.3943,14.0469, 2000);
	SetPlayerSpecialAction(otherid, SPECIAL_ACTION_NONE);
...
CMD:arrest(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1351.8241, 1570.7533, 1467.6976))
		return Error(playerid, "You must be near an arrest point.");
...
CMD:checkcitymoney(playerid, params)
{
	...
	new lstr[300];
	format(lstr, sizeof(lstr), "Keuangan Di kota Asialane: {3BBD44}%s", FormatMoney(ServerMoney));
	ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Keuangan Asialane", lstr, "Manage", "Close");
	return 1;
}
...
CMD:dropinjured(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 5.0, 1177.2365,-1309.1824,13.8858))
    {
