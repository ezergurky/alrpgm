CMD:nikahan(playerid, params[])
{
    if(pData[playerid][pAdmin] < 1)
		return ErrorMsg(playerid, "Anda bukan administrator Asialane");
    if(Nikahan == false)
    {
        Nikahan = true;
...
CMD:astream(cmdid, playerid, params[])
{
	if(pData[playerid][pAdmin] < 6)
		return ErrorMsg(playerid, "Anda bukan administrator Asia Lane");
    new link[128];
    if (sscanf(params, "s[128]", link))
    {
		SyntaxMsg(playerid, "/astream <link>");
		return 1;
	}
    new string[128], tmp[512];
    format(tmp, sizeof(tmp), "%s", link);
...
CMD:beton(playerid, params[])
{
	...
	Info(playerid, "JANGAN PAKE BETON");
	SendClientMessageToAll(COLOR_LIGHTRED, "JIKA ADA ADMIN MENGGUNAKAN BETON SILAHKAN REPORT YA");
	return 1;
}
...
CMD:acmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"Asialane:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Asialane - Bantuan Admin", line3, "OK","");
	return true;
}
...
CMD:berikansim(playerid, params[])
{
    if(pData[playerid][pAdmin] >= 6)
    {
        new to_player;
        ...
        {
            return SyntaxMsg(playerid, "/berikansim [playerid/PartOfName]");
        }

        if(!IsPlayerConnected(to_player))
        {
            return ErrorMsg(playerid, "Player tidak terhubung");
        }
        pData[to_player][pDriveLic] = 1;
        SuccesMsg(playerid, "Anda memberikan Lisensi Kendaraan kepada pemain");
        ShowItemBox(to_player, "SIM", "Mendapatkan Sim", 1581, 4);
        return 1;
    }
    else
    {
        return ErrorMsg(playerid, "Anda tidak memiliki izin untuk menggunakan perintah ini.");
    }
}

CMD:berikangps(playerid, params[])
{
    if(pData[playerid][pAdmin] >= 6)
    {
        new to_player;
        if(sscanf(params, "u", to_player))
        {
            return SyntaxMsg(playerid, "/berikangps [playerid/PartOfName]");
        }
        ...
        pData[to_player][pGPS] = 1;
        SuccesMsg(playerid, "Anda memberikan gps kepada pemain");
        ShowItemBox(to_player, "GPS", "Mendapatkan GPS", 1579, 4);
        return 1;
    }
    else
    {
...
CMD:berikanktp(playerid, params[])
{
    if(pData[playerid][pAdmin] >= 6)
    {
        new to_player;
        if(sscanf(params, "u", to_player))
        {
            return SyntaxMsg(playerid, "/berikanktp [playerid/PartOfName]");
        }
        ...
        pData[to_player][pIDCard] = 1;
        SuccesMsg(playerid, "Anda memberikan KTP kepada pemain");
        ShowItemBox(to_player, "KTP", "Mendapatkan Lisensi Bisnis", 1581, 4);
        return 1;
    }
    else
    {
...
CMD:berikanbislic(playerid, params[])
{
    if(pData[playerid][pAdmin] >= 6)
    {
        new to_player;
        if(sscanf(params, "u", to_player))
        {
            return SyntaxMsg(playerid, "/berikanbislic [playerid/PartOfName]");
        }
        ...
        SuccesMsg(playerid, "Anda memberikan Lisensi Bisnis kepada pemain");
        ShowItemBox(to_player, "Lisensi Bisnis", "Mendapatkan Lisensi Bisnis", 1277, 4);
        return 1;
    }
    else
    {
...
CMD:berikanweaplic(playerid, params[])
{
    if(pData[playerid][pAdmin] >= 6)
    {
        new to_player;
        ...
        SuccesMsg(playerid, "Anda memberikan Lisensi Senjata kepada pemain");
        ShowItemBox(to_player, "Lisensi Senjata", "Mendapatkan Lisensi Bisnis", 1277, 4);
        return 1;
    }
    else
    {
...
CMD:arelease(playerid, params[])
{
	...
	SetPlayerPositionEx(otherid, 1364.0143,1577.2247,1468.7877, 2000);
...
{
	if(pData[playerid][pAdmin] < 4)
        return ErrorMsg(playerid, "Kamu harus menjadi Admin level 4 untuk mengakses fitur ini.");
	{
...
CMD:admins(playerid, params[])
{
	...
	{
		strcat(location,"Rank\tNickname\tAdmin Name\tID\n",sizeof(location));
	if(count)
	{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS,"Staff List | Asia Lane",location,"Tutup","");
	return 1;
}
...
CMD:aduty(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] == 0)
			return ErrorMsg(playerid, "Anda bukan staff Asialane");
...
CMD:togooc(playerid, params[])
{
    ...
    if(TogOOC == 0)
    {
        SendClientMessageToAllEx(COLOR_RED, "Asialane BOT: {ffff00}Admin %s telah menonaktifkan global OOC chat.", pData[playerid][pAdminname]);
        TogOOC = 1;
    }
    else
    {
        SendClientMessageToAllEx(COLOR_RED, "Asialane BOT: {ffff00}Admin %s telah mengaktifkan global OOC chat (DON'T SPAM).", pData[playerid][pAdminname]);
        TogOOC = 0;
    }
    return 1;
}
...
CMD:o(playerid, params[])
{
    ...
    if(strlen(params) < 90)
    {
        ...
        {
            if(pData[playerid][pAdmin] == 1) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 2) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 3) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 4) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 5) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
            else if(pData[playerid][pAdmin] == 6) SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
			else if(pData[playerid][pHelper] > 0 && pData[playerid][pAdmin] == 0)
			{
				SendClientMessageEx(i, COLOR_LRED, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pAdminname], params);
			}
            else
            {
                SendClientMessageEx(i, COLOR_LRED, "[G] WARGA | %s [%d]: {ffffff}%s", pData[playerid][pUCP], playerid, params);
            }
        }
    }
    else
...
CMD:jail(playerid, params[])
{
	...
	if(reason[0] == '*')
	{
		SendClientMessageToAllEx(COLOR_LRED, "ADMINISTRATOR | %s {ffffff}telah mempenjara pemain %s selama %d bulan.", pData[playerid][pAdminname], pData[otherid][pName], timeSec);
		new str[150];
	...
	{
		SendClientMessageToAllEx(COLOR_LRED, "ADMINISTRATOR | %s telah mempenjara pemain %s selama %d bulan Alasan -> %s", pData[playerid][pAdminname], pData[otherid][pName], timeSec, reason);
		new str[150];
...
CMD:unjail(playerid, params[])
{
	...
	SetPlayerInterior(otherid, 0);
	SetPlayerVirtualWorld(otherid, 0);
	SetPlayerPos(otherid, 1508.1083,-1671.3943,14.0469);
	SetPlayerSpecialAction(otherid, SPECIAL_ACTION_NONE);

	SendClientMessageToAllEx(COLOR_LRED, "ADMINISTRATOR | %s {ffffff}telah membebaskan pemain %s dari penjara", pData[playerid][pAdminname], pData[otherid][pName]);
	new str[150];
...
CMD:kick(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_LRED, "ADMINISTRATOR | %s {FFFFFF}telah menendang %s dari kota alasan: %s.", GetName(otherid), PlayerData[playerid][pUCP], reason);
	KickEx(otherid);
    return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	new zstr[325];
	format(zstr, sizeof(zstr),"{FFFFFF}UCP Anda diblokir dari Asialane Roleplay\n\nBerdasarkan Informasi:\nNama UCP: {ffffff}%s\nIP Address: {FFFFFF}%s\nAdmin On Duty: {ffffff}%s\nTanggal Diblokir: {FFFFFF}%i/%02d/%02d %02d:%02d\nReason: {FFFFFF}%s\nDurasi Blokir: {ffffff}%s\n\n"YELLOW_E"INGAT: {FFFFFF}Jika anda ingin bermain lagi dan bertobat, masuk ke discord: "YELLOW_E"bit.ly/Asialane {ffffff}kemudian pilih chanel #request � unbanned Asialaneta", pData[playerid][pUCP], PlayerIP, PlayerData[playerid][pAdminname], date[2], date[0], date[1], date[3], date[4], tmp);
	ShowPlayerDialog(otherid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FFFFFF}Asialane - UCP Blocked", zstr, "Tutup", "");
...
CMD:setweather(playerid, params[])
{
    ...
    SuccesMsg(playerid, "Anda telah mengubah cuaca di kota Asialane.");
    return 1;
}
...
function SetName(otherplayer, playerid, nname[])
{
	if(!cache_num_rows())
	{
		...
		// {
		// 	if(!strcmp(wData[w][wOwner], oldname, true))
		// 	{
		// 		format(wData[w][wOwner], 24, "%s", newname);
...
CMD:sethpall(playerid, params[])
{
    ...
    SendClientMessageToAll(COLOR_LRED, lstr);
	foreach(new pid : Player)
	{
...
CMD:setpassword(playerid, params[])
{
	...
	if(sscanf(params, "s[21]s[20]", cname, pass))
	{
	    ...
	    Info(playerid, "Masukin nama player ngap, bukan idnya");
	   	return 1;
	}
