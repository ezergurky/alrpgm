...
CMD:bayarpajak(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5, 1444.531616,-23.858947,-55.601955))
		return Error(playerid, "Kamu harus berada di kantor sneaker");

	new string[1024];
	format(string, sizeof(string), "pajak bisnis\npajak rumah\npajak showroom\npajak vending machine");
...
------------[FUNCTION TIMER]-------------
	...
	foreach(new drid : Dealer)
	{
		...
		{
			if(DealerData[drid][dVisit] != 0 && DealerData[drid][dVisit] <= gettime())
			{
				DDealerReset(drid);
				Dealer_Save(drid);
			}
...
-----------[DIALOG.PWN]-----------
			...
			switch(listitem)
			{
				...
				case 2:
				{
					...
					Loop(itt, (count + 1), 1)
					{
						...
						if(DealerData[id][dType] == 1)
						{
							type = "Bikes Vehicles";
						}
						else if(DealerData[id][dType] == 2)
						{
							type = "Cars";
						}
						else if(DealerData[id][dType] == 3)
						{
							type = "Unique Cars";
						}
						else if(DealerData[id][dType] == 4)
						{
							type = "Job Cars";
						}
						else if(DealerData[id][dType] == 5)
						{
						...
						if(itt == count)
						{
							format(lstr,sizeof(lstr), "%d\t%s\t%s\t%s\n", id, type, GetLocation(DealerData[id][dVehX], DealerData[id][dVehY], DealerData[id][dVehZ]), ReturnTimelapse(gettime(), DealerData[id][dVisit]));
						}
						else format(lstr,sizeof(lstr), "%d\t%s\t%s\t%s\n", id, type, GetLocation(DealerData[id][dVehX], DealerData[id][dVehY], DealerData[id][dVehZ]), ReturnTimelapse(gettime(), DealerData[id][dVisit]));	
						strcat(mission,lstr,sizeof(mission));
					}
	...
	if(dialogid == DIALOG_PAYTAX_DEALER)
	{
		if(response)
		{
			...
			if(DealerData[id][dVisit] > gettime() + (86400 * 7))
			...
			DealerData[id][dVisit] = gettime() + (86400 * 15);
			Dealer_Refresh(id);
			Dealer_Save(id);

			GivePlayerMoneyEx(playerid, -15000);
			Info(playerid, "Kamu telah membayar pajak rumah (%s (ID: %d)) dengan harga "GREEN_E"$500.00 "WHITE_E"(/paytax untuk mengeceknya)", GetLocation(DealerData[id][dVehX], DealerData[id][dVehY], DealerData[id][dVehZ]), id);
		}
	}
*/