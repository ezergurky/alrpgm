...
{
	new date[6];
	TimestampToDate(EmergencyData[id][emgTime], date[2], date[1], date[0], date[3], date[4], date[5], 7);
...
stock ShowEmergency(playerid)
{
	...
	forex(i, MAX_EMERGENCY) if(EmergencyData[i][emgExists])
	{
		...
		{
			TimestampToDate(EmergencyData[i][emgTime], date[2], date[1], date[0], date[3], date[4], date[5], 7);
	...
	if(found)
		...
		ShowPlayerDialog(playerid, DIALOG_MDC_RETURN, DIALOG_STYLE_MSGBOX, "MDC - Error", "There is no recent 911 calls!", "Return", ""), PlayerPlaySound(playerid, MDC_ERROR, 0, 0, 0);
	return 1;
}
...
stock ProcessServiceCall(playerid, text[])
{
	...
	{
		new gstr[712];
		if(!strcmp(text, "sapd", true))
		{
			ServiceRequest[playerid] = SECTOR_POLICE;
			SendClientMessage(playerid, 0x1394BFFF, "911 Dispatch: 911, what's your emergency?");
		...
		{
			SendClientMessage(playerid, COLOR_BLUE, "OPERATOR:{FFFFFF} Sorry I don't quite understand you. Did you say \"sapd\" or \"samd\"?");
			return 0;
		}
	}
	...
	else if(ServiceIndex[playerid] == 3)
	{
		...
		SendClientMessageEx(playerid, 0x1394BFFF, "911 Dispatch: Okay %s %s, Can i know your current location now?", (pData[playerid][pGender] == 1) ? ("mr") : ("mrs"), ServiceName[playerid]);
	...
	else if(ServiceIndex[playerid] == 4)
	{
		...
		{
			SendFactionMessage(1, COLOR_RED, "[911] Incident: [ {FFFF00}%s {FF0000}]", ServiceInfo[playerid]);
			SendFactionMessage(1, COLOR_RED, "[911] Reporter: [ {FFFF00}%s {FF0000}] Number: [ {FFFF00}%d {FF0000}] Location: [ {FFFF00}%s {FF0000}]", ServiceName[playerid], pData[playerid][pPhone], ServiceLocation[playerid]);
		}
		else
		{
			SendFactionMessage(3, COLOR_RADIO, "[911] Reporter: %s | Number: %d", ServiceName[playerid], pData[playerid][pPhone]);
			SendFactionMessage(3, COLOR_RADIO, "[911] Description: %s", ServiceInfo[playerid]);
			SendFactionMessage(3, COLOR_RADIO, "[911] Location: %s", ServiceLocation[playerid]);
		}
	}
	return 1;
}
...
stock Emergency_Delete(id)
{
	...
	new string[64];
	mysql_format(g_SQL, string, sizeof(string), "DELETE FROM `911calls` WHERE `ID` = '%d'", EmergencyData[id][emgID]);
...
stock Emergency_Add(playerid)
{
	forex(i, MAX_EMERGENCY) if(!EmergencyData[i][emgExists])
	{
		...
		EmergencyData[i][emgIssuerID] = pData[playerid][pID];
		EmergencyData[i][emgType] = ServiceType[playerid];
		EmergencyData[i][emgTime] = gettime();
		EmergencyData[i][emgNumber] = pData[playerid][pPhone];
		mysql_tquery(g_SQL, "INSERT INTO `911calls` (`Sector`) VALUES(0)", "OnEmergencyAdded", "d", i);
		return i;
	}
	return -1;
}
...
function Emergency_Load()
{
	...
	if(rows)
	{
		...
		printf("[EMERGENCY] %d Loaded.", rows);
	}
	if(!rows)
    {
        print("[EMERGENCY] 0 Loaded.");
    }
	return 1;
}
...
{
	new query[1052];
	mysql_format(g_SQL, query, sizeof(query), "UPDATE `911calls` SET ");
	mysql_format(g_SQL, query, sizeof(query), "%s`IssuerName`='%s', ", query, EmergencyData[id][emgIssuerName]);
	mysql_format(g_SQL, query, sizeof(query), "%s`IssuerID`='%d', ", query, EmergencyData[id][emgIssuerID]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Reason`='%s', ", query, EmergencyData[id][emgReason]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Type`='%d', ", query, EmergencyData[id][emgType]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Time`='%d', ", query, EmergencyData[id][emgTime]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Sector`='%d', ", query, EmergencyData[id][emgSector]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Location`='%s', ", query, EmergencyData[id][emgLocation]);
	mysql_format(g_SQL, query, sizeof(query), "%s`Number`='%d' ", query, EmergencyData[id][emgNumber]);
	mysql_format(g_SQL, query, sizeof(query), "%sWHERE `ID` = '%d'", query, EmergencyData[id][emgID]);
