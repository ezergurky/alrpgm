...
public CarStealing(playerid)
{
    ...
    { 
		...
		{
            new query[500], brk = 1;
		  	mysql_format(g_SQL, query, sizeof(query), "UPDATE vehicle SET x='%f', y='%f', z='%f', a='%f', WHERE id='%d'", pvData[vehid][cPosX], pvData[vehid][cPosY], pvData[vehid][cPosZ], pvData[vehid][cPosA], brk, pvData[vehid][cID]);
...
CMD:chopvehicle(playerid, params[]) 
{
    ...
    {
        return Error(playerid, "Kamu ini polisi bro");
    }
    ...
    if(AdminVehicle{vehid2} || pvData[vehid2][cRent] != 0 || IsSAPDCar(vehid2) || IsGovCar(vehid2) || IsSAMDCar(vehid2) || IsSANACar(vehid2))
    {
