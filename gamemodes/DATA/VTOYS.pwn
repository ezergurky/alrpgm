...
{
	new tstr[512];
	mysql_format(g_SQL, tstr, sizeof(tstr), "SELECT * FROM toysveh WHERE Owner='%d' LIMIT 1", pvData[vehicleid][cID]);
...
MySQL_CreateVehicleToy(vehicleid)
{
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO `toysveh` (`Owner`) VALUES ('%d')", pvData[vehicleid][cID]);
	...
	{	
		...
		foreach(new i: PVehicles)
		{
			if(x == pvData[i][cVeh])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_MODTBUY, DIALOG_STYLE_LIST, "Kota Asialane - Vehicle Toys", string, "Pilih", "Batal");
			}
		}
	}
...
CMD:vtoys(playerid)
{
	...
	{	
		...
		if(x == pvData[i][cVeh])
		{
			...
			ShowPlayerDialog(playerid, DIALOG_MODTOY, DIALOG_STYLE_LIST, "Kota Asialane - Vehicle Toys", string, "Pilih", "Batal");
		}
	}
