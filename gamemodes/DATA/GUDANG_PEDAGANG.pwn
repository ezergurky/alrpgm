...
function LoadPedagang()
{
    ...
    if(rows)
    {
        for(new i; i < rows; i++)
        {
            ...
            cache_get_value_name_int(i, "Beras", pdgDATA[pid][pdgBeras]);
            cache_get_value_name_int(i, "Sambal", pdgDATA[pid][pdgSambal]);
            cache_get_value_name_int(i, "Tepung", pdgDATA[pid][pdgTepung]);
            cache_get_value_name_int(i, "Gula", pdgDATA[pid][pdgGula]);
            cache_get_value_name_int(i, "Bijikopi", pdgDATA[pid][pdgBijikopi]);
            cache_get_value_name_int(i, "Sariteh", pdgDATA[pid][pdgSariteh]);
            cache_get_value_name_int(i, "Sarijeruk", pdgDATA[pid][pdgSarijeruk]);
...
{
    new cQuery[512];
    format(cQuery, sizeof(cQuery), "UPDATE pedagang SET type='%d', posx='%f', posy='%f', posz='%f', interior='%d', ayamfilet='%d', Beras='%d', Sambal='%d', Tepung='%d', Gula='%d', Bijikopi='%d', Sariteh='%d', Sarijeruk='%d', Mineral='%d' WHERE id='%d'",
    pdgDATA[pid][pdgType],
    pdgDATA[pid][pdgPosX],
    pdgDATA[pid][pdgPosY],
    pdgDATA[pid][pdgPosZ],
