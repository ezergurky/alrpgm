...
function LoadPlayerVehicle(playerid)
{
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z++)
		{
			...
			// mysql_tquery(g_SQL, string, "Vehicle_ObjectLoaded", "dd", i, playerid);
		}
...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			if(pvData[i][cVeh] != 0)
			{
				DisableVehicleSpeedCap(pvData[i][cVeh]);
...
// panel radial menu aufa
CMD:radialmenu(playerid, params[])
{	
		//click panel aufa
        ...
        {
            PlayerTextDrawShow(playerid, BODYRADIAL[playerid][txd]);
            SelectTextDraw(playerid, 0xFF0000FF);
        }
		return 1;
}
...
CMD:apv(playerid, params[])
{
		...
		if(found)
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Asialane - Kendaraan Player", "Player ini tidak memeliki kendaraan", "Close", "");
		return 1;
}
...
CMD:vrm(playerid, params[])
{
	...
	if((carid = Vehicle_Nearest(playerid)) != -1)
	{
		...
		{
    		ShowPlayerDialog(playerid, DIALOG_VRM, DIALOG_STYLE_LIST, "Asialane - Menu Kendaraan", "Kunci\nLampu\nBagasi\nBuka/Tutup Hood\nBuka/Tutup Trunk", "Pilih", "Batal");
		}
...
CMD:myv(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_MYVEH, DIALOG_STYLE_TABLIST_HEADERS, "{7fffd4}Asialane {ffffff}- Kepemilikan Kendaraan", CMDSString, "Pilih", "Batal");
	return 1;
}
