...
{		
	...
	if(GetNearestVehicleToPlayer(playerid, 3.5, false) == pData[playerid][pMechVeh])
	{
		...
		ShowPlayerDialog(playerid, DIALOG_SERVICE, DIALOG_STYLE_TABLIST_HEADERS, "Asialane - Service", Dstring, "Service", "Batal");
	}
	else
	{
...
{
	if(!IsPlayerConnected(playerid) || pData[playerid][pMechanicStatus] != 1)
		return 1;
...
function NeonCar(playerid, vehicleid)
{
	...
	if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
	{
		if(IsValidVehicle(vehicleid))
		{
			...
			else SetVehicleNeonLights(vehicleid, true, pData[playerid][pMechColor1], 0);
