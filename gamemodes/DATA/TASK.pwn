...
{	
	...
	if(up_seconds == 60)
	{
	    ...
	    if(up_minutes == 60)
	    {
			...
			format(tstr, sizeof tstr, "[:clock:]: Server Telah Online selama %s.", Uptime());
			DCC_AddEmbedField(msgEmbed, "Asialane Uptime", tstr);
			format(tstr, sizeof tstr, "[:7436online:]: Player di dalam kota %s/%d.", number_format(Iter_Count(Player)), GetMaxPlayers());
			DCC_AddEmbedField(msgEmbed, "Player Asialane", tstr);
			format(tstr, sizeof tstr, "[:calendar_spiral:]: Server Online Dari %s, Dan Sampai Sekarang %s.", dateNow, timeNow);
			DCC_AddEmbedField(msgEmbed, "Waktu Asialane", tstr);
			DCC_SetEmbedColor(msgEmbed, 0xffa500);
			DCC_SendChannelEmbedMessage(g_Discord_Serverstatus, logsstatus);
