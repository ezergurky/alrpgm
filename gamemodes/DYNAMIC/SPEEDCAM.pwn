...
function LoadSpeedCam()
{
	...
	if(rows)
	{
	    for(new i; i < rows; i++)
	    {
	        ...
	        new label[200];
	        format(label, sizeof label, "{FFFF00}[SpeedCam]\n{FF0000}Max Speed: {FFFF00}%.0f {ffffff}KM/H", CamData[sid][CamLimit]);
...
CMD:createsc(playerid, params[])
{
	...
	if (sscanf(params, "ff", limit, range))
		return SyntaxMsg(playerid, "/createspeedcam [speed limit] [range] (default range: 30)");
...
CMD:editsc(playerid, params[])
{
	...
	{
		SyntaxMsg(playerid, "/editsc [id] [name]");
	...
	{			
		if(sscanf(param, "i", ammount))
			return SyntaxMsg(playerid, "/editsc [id] [speed] [ammount]");
	...
	{			
		if(sscanf(param, "i", ammount))
			return SyntaxMsg(playerid, "/editsc [id] [range] [ammount]");
...
CMD:deletesc(playerid, params[])
{
	...
	new id, query[512];
	if(sscanf(params, "i", id)) return SyntaxMsg(playerid, "/deletesc [id]");
	if(!Iter_Contains(Cem, id)) return Error(playerid, "Invalid ID.");
...
CMD:gotosc(playerid, params[])
{
	...
	if(sscanf(params, "d", id))
		return SyntaxMsg(playerid, "/gotosc [id]");
