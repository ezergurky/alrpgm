...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(strcmp(bData[id][bOwner], "-") || bData[id][bOwnerID] != 0)
		{
			...
			bData[id][bLabelPoint] = CreateDynamic3DTextLabel(tstr, COLOR_YELLOW, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
   			if(IsValidDynamic3DTextLabel(bData[id][bLabel]))
            DestroyDynamic3DTextLabel(bData[id][bLabel]);
        }
        else
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "Asia Lane - Bisnis Saya", CMDSString, "Pilih", "Batal");
	return 1;
}
...
CMD:bm(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, BISNIS_MENU, DIALOG_STYLE_LIST, "Asia Lane - Bisnis Menu","Bisnis Info\nGanti Nama\nPendapatan Bisnis\nHarga Product\nRequest Restock","Pilih","Tutup");
    return 1;
}
