...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"{BABABA}%s",dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(1318, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint], -1, 50);
		...
		else
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"{BABABA}%s",dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(19606, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint], -1, 50);
			dData[id][dLabelext] = CreateDynamic3DTextLabel(mstr, COLOR_YELLOW, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]+0.35, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dExtvw], dData[id][dExtint]);
		}
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dPickupint] = CreateDynamicPickup(19606, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 50);
			}
			else
			{
				new mstr[512];
...
CMD:createdoor(playerid, params[])
{
	...
	format(mstr,sizeof(mstr),"{BABABA}%s", dData[did][dName]);
	dData[did][dPickupext] = CreateDynamicPickup(19197, 23, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ], dData[did][dExtvw], dData[did][dExtint], -1, 50);
