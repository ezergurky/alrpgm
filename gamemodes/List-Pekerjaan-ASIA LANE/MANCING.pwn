...
CMD:mancing(playerid,params[])
{
	if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if(pData[playerid][pUmpanaufa] < 1) return ErrorMsg(playerid, "Anda tidak memiliki umpan");
	if(pData[playerid][pPancinganaufa] < 1) return ErrorMsg(playerid, "Anda tidak memiliki pancingan");
	new random2 = RandomEx(10000, 20000);
	pData[playerid][pInFish] = 1;
	SetTimerEx("FishTime", random2, 0, "i",playerid);
	ShowProgressbar(playerid, "Memancing ikan", 15);
...
CMD:jualmarakel(playerid, params[])
{
...
CMD:jualbluefish(playerid, params[])
{
