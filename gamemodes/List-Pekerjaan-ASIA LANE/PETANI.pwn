...
//hasil tani minuman pedagang
function NanamKopi(playerid)
{
	pData[playerid][pOlahkopi] += 5;
	ShowItemBox(playerid, "OlahKopi", "Received_5x", 742, 2);
}
function NanamTeh(playerid)
{
	pData[playerid][pOlahteh] += 5;
	ShowItemBox(playerid, "OlahTeh", "Received_5x", 2247, 2);
}
function NanamJeruk(playerid)
{
	pData[playerid][pOlahjeruk] += 5;
	ShowItemBox(playerid, "OlahJeruk", "Received_5x", 741, 2);
}

function ProsesKopi(playerid)
{
	pData[playerid][pBijikopi] += 5;
	ShowItemBox(playerid, "Bijikopi", "Received_5x", 2880, 2);
}
function ProsesSariteh(playerid)
{
	pData[playerid][pSariteh] += 5;
	ShowItemBox(playerid, "SariTeh", "Received_5x", 2353, 2);
}
function ProsesSarijeruk(playerid)
{
	pData[playerid][pSarijeruk] += 5;
	ShowItemBox(playerid, "SariJeruk", "Received_5x", 19574, 2);
}
...
CMD:jualsambal(playerid, params[])
{
...
CMD:jualtepung(playerid, params[])
{
...
CMD:jualkopi(playerid, params[])
{
    new total = pData[playerid][pBijikopi];
    if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
    if(pData[playerid][pBijikopi] < 5) return ErrorMsg(playerid, "Anda Tidak Memiliki 1 paket Biji kopi");
    ShowProgressbar(playerid, "Menjual Biji Kopi..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new pay = pData[playerid][pBijikopi] * 3;
	GivePlayerMoneyEx(playerid, pay);
	pData[playerid][pBijikopi] -= total;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", total);
	ShowItemBox(playerid, "Biji Kopi", str, 2880, 4);
    Inventory_Update(playerid);
	return 1;
}
CMD:jualteh(playerid, params[])
{
    new total = pData[playerid][pSariteh];
    if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
    if(pData[playerid][pSariteh] < 5) return ErrorMsg(playerid, "Anda Tidak Memiliki 1 paket sari teh");
    ShowProgressbar(playerid, "Menjual Sari teh...", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new pay = pData[playerid][pSariteh] * 2;
	GivePlayerMoneyEx(playerid, pay);
	pData[playerid][pSariteh] -= total;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", total);
	ShowItemBox(playerid, "Sari Teh", str, 2353, 4);
    Inventory_Update(playerid);
	return 1;
}
CMD:jualjeruk(playerid, params[])
{
    new total = pData[playerid][pSarijeruk];
    if(pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
    if(pData[playerid][pSarijeruk] < 5) return ErrorMsg(playerid, "Anda Tidak Memiliki 1 paket sari jeruk");
    ShowProgressbar(playerid, "Menjual Sari jeruk...", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new pay = pData[playerid][pSarijeruk] * 2;
	GivePlayerMoneyEx(playerid, pay);
	pData[playerid][pSarijeruk] -= total;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", total);
	ShowItemBox(playerid, "Sari jeruk", str, 19574, 4);
    Inventory_Update(playerid);
	return 1;
}
