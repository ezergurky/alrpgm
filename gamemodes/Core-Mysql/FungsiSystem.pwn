...
{
	InterpolateCameraPos(playerid, 356.1173,-2017.6241,59.5162, 199.0929,-1919.1708,39.9441, 60000);
	InterpolateCameraLookAt(playerid, 356.1173,-2017.6241,59.5162, 199.0929,-1919.1708,39.9441, 50000);
	...
	if(cache_num_rows() > 0)
	{
		...
		{
			new str[400];
			format(str, sizeof(str), "UCP: {15D4ED}%s\n{ffffff}Silahkan masukkan PIN yang sudah di kirimkan oleh Asia Lane BOT", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "{BABABA}Asia Lane - {ffffff}Verify Account", str, "Input", "Batal");
		}
		if(pData[playerid][pPassword] > 10)
		{
			new lstring[512];
			format(lstring, sizeof lstring, "Selamat datang kembali di server Asia Lane Roleplay\n\nUsername: %s\nVersion: ALRP v1.8.9\n{FFFF00}(Silahkan Masukkan Kata Sandi Anda Di Bawah Ini:)", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login - Asia Lane", lstring, "Masuk", "Keluar");
		}
		pData[playerid][LoginTimer] = SetTimerEx("OnLoginTimeout", SECONDS_TO_LOGIN * 1000, false, "i", playerid);
	}
	else
	{
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX,"Kota Asia Lane - Tiket Karcis","Dari: Penjaga Pintu Asia Lane #1\nKepada: Calon Aktor (Pemain Peran) di Kota Asia Lane\n\nSilahkan Terlebih Dahulu mengambil tiket pintu Asia Lane #1 di discord sebelum dapat memasuki Kota Asia Lane\nLink Discord: bit.ly/Asia Lane","Keluar","");
...
function UpdateNametagAdmin()
{
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
    {
        if(IsPlayerConnected(i))
        {
            ...
            if(armour > 1.0)
            {
                format(nametag, sizeof(nametag), "{BABABA}Sedang Bertugas\n%s [%d]\n"YELLOW_E"%s", pData[i][pAdminname], i, GetStaffRank(i));
            }
            else
            {
                format(nametag, sizeof(nametag), "{BABABA}Sedang Bertugas\n%s [%d]\n"YELLOW_E"%s", pData[i][pAdminname], i, GetStaffRank(i));
...
{
    printf("Sebelum UpdatePlayerData");
    UpdatePlayerData(playerid);
    printf("Setelah UpdatePlayerData");
    return 1;
}
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "Kompensasi1", pData[playerid][pKompensasi1]);
	cache_get_value_name_int(0, "Kompensasi2", pData[playerid][pKompensasi2]);
	...
	cache_get_value_name_int(0, "togname", pData[playerid][pTogName]);
	...
	//enum pedagang menu masak
	cache_get_value_name_int(0, "nasikucing", pData[playerid][pNasikucing]);
	cache_get_value_name_int(0, "ayamgeprek", pData[playerid][pAyamgeprek]);
	cache_get_value_name_int(0, "Kue", pData[playerid][pKueaufa]);
	...
	cache_get_value_name_int(0, "umpan", pData[playerid][pUmpanaufa]);
	cache_get_value_name_int(0, "pancingan", pData[playerid][pPancinganaufa]);
	...
	//bibit
	cache_get_value_name_int(0, "benihkopi", pData[playerid][pBenihkopi]);
	cache_get_value_name_int(0, "benihteh", pData[playerid][pBenihteh]);
	cache_get_value_name_int(0, "benihjeruk", pData[playerid][pBenihjeruk]);
	//hasil olah
	cache_get_value_name_int(0, "olahkopi", pData[playerid][pOlahkopi]);
	cache_get_value_name_int(0, "olahteh", pData[playerid][pOlahteh]);
	cache_get_value_name_int(0, "olahjeruk", pData[playerid][pOlahjeruk]);
	//barang
	cache_get_value_name_int(0, "bijikopi", pData[playerid][pBijikopi]);
	cache_get_value_name_int(0, "sariteh", pData[playerid][pSariteh]);
	cache_get_value_name_int(0, "sarijeruk", pData[playerid][pSarijeruk]);

	//menu minum pedagng
	cache_get_value_name_int(0, "kopiruq", pData[playerid][pKopiruq]);
	cache_get_value_name_int(0, "esteh", pData[playerid][pEsteh]);
	cache_get_value_name_int(0, "esjeruk", pData[playerid][pEsjeruk]);

	//bandana 
	cache_get_value_name_int(0, "bandanaruq", pData[playerid][pBandana]);
	...
	if(pData[playerid][pJail] == 1)
	{
	    for(new i = 0; i < 20; i++)
		{
			TextDrawHideForPlayer(playerid, Spawnaufa[i]);
	...
	connect = DCC_FindChannelById("1204853723579424868");
	DCC_SetEmbedTitle(logss, "Asia Lane Roleplay");
	DCC_SetEmbedTimestamp(logss, "Team Service Asia Lane");
	...
	new stroi[5000];
	format(stroi, sizeof(stroi), "**s** telah masuk ke dalam server Asia Lane\n**UCP: %s**\n**Player ID:** %d\n**Negara:** Indonesia", pData[playerid][pName], pData[playerid][pUCP], playerid);
	DCC_AddEmbedField(logss, "Asia Lane Development Team", stroi, true);
...
function CekNamaDobelJing(playerid, name[])
{
	...
	if(rows > 0)
	{
		ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, "Create Character", "ERROR: Nama tersebut sudah di miliki orang!\nMasukkan nama baru kamu\n\nExample: Faruq_Jumantara.", "Create", "Back");
	}
	else 
	{
		...
		format(AtmInfo,1000,"%s", name);
	   	PlayerTextDrawSetString(playerid, RegisterErn[playerid][18], AtmInfo);
	}
}
...
function SafeLogin(playerid)
{
    ...
    //     Error(playerid, "Sebagai contoh, Faruq_Jumantara, Yann_Jumantara dll.");
    //     KickEx(playerid);
    // }
}
...
stock InfoTD_MSG(playerid, ms_time, text[])
{
...
function pCountDown()
{
	...
	// {
	// 	if(strcmp(DealerData[drid][dealerOwner], "-"))
	// 	{
	// 		if(DealerData[drid][dealerRestock] != 0 && DealerData[drid][dealerRestock] <= gettime())
	// 		{
	// 			DealerReset(drid);
	// 			DealerSave(drid);
	// 		}
